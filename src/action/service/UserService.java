...
public class UserService extends BaseService {
    ...
    public static String addStaffInfo(String jsonString, int userId) {
        ...
        String rsTest = "";
        if (jsonObject.get("account_number").equals("")) {
            rsTest = "请填写账号";
        } else if (jsonObject.get("login_password").equals("")) {
            rsTest = "请填写密码";
        }
//        else if (jsonObject.get("email").equals("")) {
//            rsTest = "请填写邮箱";
//        }
        else if (jsonObject.get("phone_num").equals("")) {
            rsTest = "请填写手机号";
        } else if (jsonObject.get("department").equals("0")) {
            rsTest = "请选择部门";
        } else if (jsonObject.get("account_number").toString().length()<6) {
            rsTest = "账号长度大于6";
        } else if (jsonObject.get("login_password").toString().length()<6){
            rsTest = "密码长度大于6";
        }
        else {
...
//			int create_time= StringHandler.getLongDate();
            ...
            if (gaia_id != 0) {
                addId = sendObjectCreate(227, nick_name, gaia_id, department, job_number, email, phone_num, 1, 1, role, memo);
...
//            gaia_id = (int) UserService.getFieldValue(res, "ids", Integer.class);
    ...
    public static Integer getMaxJob_number(String department) {
        int addSid = sendObject(225, department);
    ...
    public static String getStaffInfoList(int begin, int end, String login_name, String nick_name, String role, String department) {
        ...
        int sid = sendObjectBase(229, sql, begin, end);
...
//			int create_time= StringHandler.getLongDate();
    ...
    public static String verify(String jsonString, HttpServletRequest request) {
        JSONObject jsonObject = JSONObject.parseObject(jsonString);
        String oPwd = jsonObject.get("old_password").toString();
...
//        String jobPone = jsonObject.get("job_phone").toString();
        ...
        //  默认已经拥有gaia_id
        try {
            depId = jsonObject.get("depId").toString();
        ...
        if (flag) {
            ...
            String gaia_id_temp = jsonObject.get("gaia_id").toString();
            gaia_id = Integer.valueOf(gaia_id_temp);
            // 此处执行为员工修改登录密码
            // 员工登录密码修改
            String login_name = jsonObject.get("login_name").toString();
            String  res = updateGainaUserInfo(login_name,oPwd,gaia_id+"");
            // 直接由登录人输入员工新密码 更新
            String nick_name = jsonObject.get("nick_name").toString();
            String memo = jsonObject.get("memo").toString();
            String id = jsonObject.get("id").toString();
            int updateId = sendObjectCreate(332, nick_name, depId, email, phone, roleId, memo,"",id);
    ...
    public static String updateUser(String phone, String email, String job_number, int userId) {
        int uId = sendObjectCreate(344, phone, email, job_number, userId);
        String res = ResultPoor.getResult(uId);
        JSONObject resJson = JSONObject.parseObject(res);
        String resJsonSuccess = resJson.get("success") == null ? "" : (resJson.get("success").toString());
        return resJsonSuccess;
